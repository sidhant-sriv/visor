...
export class PyAstParser extends AbstractParser {
    ...
    public generateFlowchart(sourceCode: string, functionName?: string, position?: number): FlowchartIR {
        ...
        if (position !== undefined) {
            ...
            const smallestStatement = statements.reduce<Parser.SyntaxNode | undefined>((smallest, stmt) => {
                if (position >= stmt.startIndex && position <= stmt.endIndex && (!smallest || stmt.endIndex - stmt.startIndex < smallest.endIndex - smallest.startIndex)) {
                    return stmt;
                }
                return smallest;
            }, undefined);
            ...
            if (smallestStatement) {
                ...
                if (potentialCallNode?.type === "call") {
                    ...
                    const funcNameText = funcNode?.text;
                    const args = potentialCallNode.childForFieldName("arguments")?.namedChildren || [];

                    if (["list", "tuple", "set"].includes(funcNameText!) && args.length === 1 && args[0].type === "call" && this.isHofCall(args[0])) {
                        return this.generateHofFlowchart(baseStatement, args[0], funcNameText);
        ...
        if (position !== undefined) {
            targetNode = tree.rootNode.descendantsOfType("function_definition").find((f) => position >= f.startIndex && position <= f.endIndex);
            if (!targetNode) {
                targetNode = tree.rootNode.descendantsOfType("assignment").find((a) => position >= a.startIndex && position <= a.endIndex && a.childForFieldName("right")?.type === "lambda");
                isLambda = !!targetNode;
            }
        } else if (functionName) {
            targetNode = tree.rootNode.descendantsOfType("function_definition").find((f) => f.childForFieldName("name")?.text === functionName);
            if (!targetNode) {
                targetNode = tree.rootNode.descendantsOfType("assignment").find((a) => a.childForFieldName("left")?.text === functionName && a.childForFieldName("right")?.type === "lambda");
                isLambda = !!targetNode;
            }
        } else {
        ...
        if (!targetNode) {
            return { nodes: [{ id: "A", label: "Place cursor inside a function or statement to generate a flowchart.", shape: "rect" }], edges: [], locationMap: [] };
        }

        this.currentFunctionIsLambda = isLambda;
        const bodyToProcess = isLambda ? targetNode.childForFieldName("right")!.childForFieldName("body") : targetNode.childForFieldName("body");
        const funcNameStr = this.escapeString(isLambda ? targetNode.childForFieldName("left")!.text : targetNode.childForFieldName("name")!.text);
        const title = `Flowchart for ${isLambda ? "lambda" : "function"}: ${funcNameStr}`;

        if (!bodyToProcess) {
            return { nodes: [{ id: "A", label: "Function has no body.", shape: "rect" }], edges: [], locationMap: [] };
        }
        ...
        const bodyResult = isLambda ? this.processStatement(bodyToProcess, exitId) : this.processBlock(bodyToProcess, exitId);
        nodes.push(...bodyResult.nodes);
        edges.push(...bodyResult.edges);

        edges.push(bodyResult.entryNodeId ? { from: entryId, to: bodyResult.entryNodeId } : { from: entryId, to: exitId });
    ...
    protected processStatement(statement: Parser.SyntaxNode, exitId: string, loopContext?: LoopContext, finallyContext?: { finallyEntryId: string }): ProcessResult {
        // FIX: Recursively process the content of a parenthesized expression.
        if (statement.type === "parenthesized_expression" && statement.namedChild(0)) {
        ...
        switch (statement.type) {
            ...
            case "pass_statement": return this.createProcessResult();
            default: {
                ...
                if (expressionNode) {
                    if (expressionNode.type === "conditional_expression" && assignmentTargetNode) {
                        ...
                        const nodes: FlowchartNode[] = [{ id: conditionId, label: this.escapeString(conditionNode.text), shape: "diamond", style: this.nodeStyles.decision }];
                        const edges: FlowchartEdge[] = [];
                        ...
                        nodes.push({ id: consequenceId, label: `${targetText} = ${this.escapeString(consequenceNode.text)}`, shape: "rect", style: this.nodeStyles.process });
                        ...
                        nodes.push({ id: alternativeId, label: `${targetText} = ${this.escapeString(alternativeNode.text)}`, shape: "rect", style: this.nodeStyles.process });
                        ...
                        return this.createProcessResult(nodes, edges, conditionId, [{ id: consequenceId }, { id: alternativeId }]);
                    }
                    ...
                    if (hofInfo) {
                        ...
                        if (hofResult) {
                            ...
                            if (hofInfo.containerName) {
                                ...
                                allNodes.push({ id: convertId, label: `Convert to ${hofInfo.containerName}`, shape: "rect", style: this.nodeStyles.process });
                            ...
                            if (assignmentTargetNode) {
                                ...
                                allNodes.unshift({ id: assignId, label: `${this.escapeString(assignmentTargetNode.text)} = ...`, shape: "rect", style: this.nodeStyles.process });
                                if (currentEntry) allEdges.unshift({ from: assignId, to: currentEntry });
                                currentEntry = assignId;
                            }

                            return this.createProcessResult(allNodes, allEdges, currentEntry, currentExits);
                        }
                    }
                }
    ...
    private generateHofFlowchart(statementNode: Parser.SyntaxNode, hofCallNode: Parser.SyntaxNode, containerName?: string): FlowchartIR {
        ...
        if (["assignment", "expression_statement"].includes(statementNode.type)) {
            ...
            nodes.unshift({ id: assignId, label: `${leftText} = ...`, shape: "rect", style: this.nodeStyles.process });
            if (entryPointId) edges.unshift({ from: assignId, to: entryPointId });
            entryPointId = assignId;
            this.locationMap.push({ start: assignment.childForFieldName("left")!.startIndex, end: assignment.childForFieldName("left")!.endIndex, nodeId: assignId });
        }
    
        if (containerName) {
            const convertId = this.generateNodeId("convert");
            nodes.push({ id: convertId, label: `Convert to ${containerName}`, shape: "rect", style: this.nodeStyles.process });
        ...
        return { nodes, edges, locationMap: this.locationMap, functionRange: { start: statementNode.startIndex, end: statementNode.endIndex }, title: `Flowchart for: ${this.escapeString(statementNode.text)}`, entryNodeId: startId, exitNodeId: endId };
    }

    private processConditionalExpression(condExprNode: Parser.SyntaxNode, exitId: string, loopContext?: LoopContext, finallyContext?: { finallyEntryId: string }): ProcessResult {
        ...
        return this.createProcessResult(nodes, edges, conditionId, this.currentFunctionIsLambda ? [] : [...consequenceResult.exitPoints, ...alternativeResult.exitPoints], nodesConnectedToExit);
    }

    private processIfStatement(ifNode: Parser.SyntaxNode, exitId: string, loopContext?: LoopContext, finallyContext?: { finallyEntryId: string }): ProcessResult {
        const ifConditionNode = ifNode.childForFieldName("condition");
        const ifConsequenceNode = ifNode.childForFieldName("consequence");
        if (!ifConditionNode || !ifConsequenceNode) return this.createProcessResult();

        const ifConditionId = this.generateNodeId("cond");
        const nodes: FlowchartNode[] = [{ id: ifConditionId, label: this.escapeString(ifConditionNode.text), shape: "diamond", style: this.nodeStyles.decision }];
        ...
        for (const clause of alternatives) {
            if (clause.type === "elif_clause") {
                ...
                nodes.push({ id: elifConditionId, label: this.escapeString(elifConditionNode.text), shape: "diamond", style: this.nodeStyles.decision });
        ...
        return this.createProcessResult(nodes, edges, ifConditionId, allExitPoints, nodesConnectedToExit);
    }
    ...
    private processForStatement(forNode: Parser.SyntaxNode, exitId: string, finallyContext?: { finallyEntryId: string }): ProcessResult {
        ...
        const nodes: FlowchartNode[] = [
            { id: headerId, label: this.escapeString(`for ${left} in ${right}`), shape: "diamond", style: this.nodeStyles.decision },
        ...
        return this.createProcessResult(nodes, edges, headerId, [{ id: loopExitId }], bodyResult.nodesConnectedToExit);
    }
    ...
    private processWhileStatement(whileNode: Parser.SyntaxNode, exitId: string, finallyContext?: { finallyEntryId: string }): ProcessResult {
        ...
        const nodes: FlowchartNode[] = [
            { id: conditionId, label: conditionText, shape: "diamond", style: this.nodeStyles.decision },
            { id: loopExitId, label: "end loop", shape: "stadium" },
        ];
        ...
        return this.createProcessResult(nodes, edges, conditionId, [{ id: loopExitId }], bodyResult.nodesConnectedToExit);
    }

    private processReturnStatement(returnNode: Parser.SyntaxNode, exitId: string, finallyContext?: { finallyEntryId: string }): ProcessResult {
        ...
        if (valueNode) {
            ...
            if (hofInfo) {
                ...
                if (hofResult) {
                    ...
                    allNodes.push({ id: returnId, label: "return result", shape: "stadium", style: this.nodeStyles.special });
                    ...
                    return this.createProcessResult(allNodes, allEdges, hofResult.entryNodeId, [], new Set([returnId]));
                }
            }
        }
        ...
        const node: FlowchartNode = { id: nodeId, label: labelText, shape: "stadium", style: this.nodeStyles.special };
        const edges: FlowchartEdge[] = [{ from: nodeId, to: finallyContext ? finallyContext.finallyEntryId : exitId }];
        this.locationMap.push({ start: returnNode.startIndex, end: returnNode.endIndex, nodeId });
        return this.createProcessResult([node], edges, nodeId, [], new Set([nodeId]));
    }

    private processRaiseStatement(raiseNode: Parser.SyntaxNode, exitId: string, finallyContext?: { finallyEntryId: string }): ProcessResult {
        const nodeId = this.generateNodeId("raise");
        const valueNodes = raiseNode.namedChildren;
        const labelText = valueNodes.length > 0 ? `raise ${this.escapeString(valueNodes.map((n) => n.text).join(", "))}` : "raise";
    ...
    private processMatchStatement(matchNode: Parser.SyntaxNode, exitId: string, loopContext?: LoopContext, finallyContext?: { finallyEntryId: string }): ProcessResult {
        ...
        const nodes: FlowchartNode[] = [{ id: subjectId, label: `match ${this.escapeString(subjectNode.text)}`, shape: "rect", style: this.nodeStyles.process }];
        ...
        for (const clause of caseClauses) {
            ...
            nodes.push({ id: caseConditionId, label: caseLabel, shape: "diamond", style: this.nodeStyles.decision });
        ...
        return this.createProcessResult(nodes, edges, subjectId, allExitPoints, nodesConnectedToExit);
    }

    private processTryStatement(tryNode: Parser.SyntaxNode, exitId: string, loopContext?: LoopContext, finallyContext?: { finallyEntryId: string }): ProcessResult {
        ...
        return this.createProcessResult(nodes, edges, entryId, allExitPoints, nodesConnectedToExit);
    }
    
    private processWithStatement(withNode: Parser.SyntaxNode, exitId: string, loopContext?: LoopContext, finallyContext?: { finallyEntryId: string }): ProcessResult {
        ...
        const nodes: FlowchartNode[] = [{ id: withEntryId, label: this.escapeString(withClauseNode?.text || "with ..."), shape: "rect", style: this.nodeStyles.special }];
        ...
        if (body) {
            ...
            return this.createProcessResult(nodes, edges, withEntryId, bodyResult.exitPoints.length > 0 ? bodyResult.exitPoints : [{ id: withEntryId }], bodyResult.nodesConnectedToExit);
        }
    
        return this.createProcessResult(nodes, edges, withEntryId, [{ id: withEntryId }]);
    }
    
    private processAssertStatement(assertNode: Parser.SyntaxNode, exitId: string, loopContext?: LoopContext, finallyContext?: { finallyEntryId: string }): ProcessResult {
        ...
        const nodes: FlowchartNode[] = [{ id: conditionId, label: `assert ${this.escapeString(conditionNode.text)}`, shape: "diamond", style: this.nodeStyles.decision }];
        ...
        nodes.push({ id: raiseNodeId, label, shape: "stadium", style: this.nodeStyles.special });
        ...
        return this.createProcessResult(nodes, edges, conditionId, [{ id: conditionId, label: "True" }], new Set([raiseNodeId]));
    }
    ...
    private processArgument(argNode: Parser.SyntaxNode): ProcessResult {
        if (this.isHofCall(argNode)) {
            ...
            const node: FlowchartNode = { id: nodeId, label: `Input: ${this.escapeString(argNode.text)}`, shape: "rect", style: this.nodeStyles.special };
    ...
    private processMap(callNode: Parser.SyntaxNode): ProcessResult {
        ...
        const lambdaBodyText = functionArg.type === "lambda" ? this.escapeString(functionArg.childForFieldName("body")!.text) : `${functionText}(item)`;
        ...
        nodes.push({ id: mapId, label: `map()`, shape: "rect", style: this.nodeStyles.hof });
        ...
        nodes.push({ id: applyId, label: `Apply lambda to each element: new_item = ${lambdaBodyText}`, shape: "rect", style: this.nodeStyles.process });
        ...
        nodes.push({ id: collectId, label: "Collect transformed element", shape: "rect", style: this.nodeStyles.process });
        edges.push({ from: applyId, to: collectId });
        edges.push({ from: collectId, to: mapId });
    
        const resultId = this.generateNodeId("map_result");
        nodes.push({ id: resultId, label: "Collected results", shape: "rect", style: this.nodeStyles.special });
        edges.push({ from: mapId, to: resultId, label: "End of list" });
    
        return this.createProcessResult(nodes, edges, iterableResult.entryNodeId, [{ id: resultId }]);
    }
    ...
    private processFilter(callNode: Parser.SyntaxNode): ProcessResult {
        ...
        nodes.push({ id: filterId, label: `filter()`, shape: "rect", style: this.nodeStyles.hof });
        ...
        nodes.push({ id: applyId, label: `Apply lambda to each element`, shape: "rect", style: this.nodeStyles.process });
        this.locationMap.push({ start: functionArg.startIndex, end: functionArg.endIndex, nodeId: applyId });
        edges.push({ from: filterId, to: applyId, label: "Next item" });
    
        const decisionId = this.generateNodeId("filter_decision");
        nodes.push({ id: decisionId, label: `lambda returns True?`, shape: "diamond", style: this.nodeStyles.decision });
        edges.push({ from: applyId, to: decisionId });
    
        const keepId = this.generateNodeId("filter_keep");
        nodes.push({ id: keepId, label: "Keep element", shape: "rect", style: this.nodeStyles.process });
        ...
        nodes.push({ id: discardId, label: "Discard element", shape: "rect", style: this.nodeStyles.break });
        ...
        nodes.push({ id: collectedId, label: "Collected results", shape: "rect", style: this.nodeStyles.special });
        edges.push({ from: filterId, to: collectedId, label: "End of list" });
    
        return this.createProcessResult(nodes, edges, iterableResult.entryNodeId, [{ id: collectedId }]);
    }
    ...
    private processReduce(callNode: Parser.SyntaxNode): ProcessResult {
        ...
        const initializerText = initializerArgNode ? this.escapeString(initializerArgNode.text) : `first item from input`;
        ...
        nodes.push({ id: initId, label: `accumulator = ${initializerText}`, shape: "rect", style: this.nodeStyles.process });
        ...
        nodes.push({ id: headerId, label: hasInitializer ? `For each item` : `For each remaining item`, shape: "rect", style: this.nodeStyles.hof });
        edges.push({ from: initId, to: headerId });
    
        const applyId = this.generateNodeId("reduce_apply");
        nodes.push({ id: applyId, label: `accumulator = ${this.escapeString(functionArg.text)}(accumulator, item)`, shape: "rect", style: this.nodeStyles.process });
        this.locationMap.push({ start: functionArg.startIndex, end: functionArg.endIndex, nodeId: applyId });
        ...
        nodes.push({ id: resultId, label: "Return final accumulator value", shape: "rect", style: this.nodeStyles.special });
        edges.push({ from: headerId, to: resultId, label: "End" });
    
        return this.createProcessResult(nodes, edges, iterableResult.entryNodeId, [{ id: resultId }]);
    }

    private processReturnStatementForExpression(exprNode: Parser.SyntaxNode, exitId: string, finallyContext?: { finallyEntryId: string }): ProcessResult {
        const nodeId = this.generateNodeId("return");
