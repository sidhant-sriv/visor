...
export class PyAstParser extends AbstractParser {
  ...
  public generateFlowchart(
    ...
    const tree = this.parser.parse(sourceCode);
    this.sourceCode = sourceCode; // Store source code for syntax highlighting
    this.resetState();
    ...
    if (position !== undefined) {
      ...
      if (smallestStatement) {
        ...
        if (potentialCallNode?.type === "call") {
          ...
          if (
            ["list", "tuple", "set"].includes(funcNameText!) &&
            args.length === 1 &&
            args[0].type === "call" &&
      ...
      default: {
        ...
        if (expressionNode) {
          if (
            ...
            const [
              consequenceSyntaxNode,
              conditionSyntaxNode,
              alternativeSyntaxNode,
            ] = namedChildren;
            ...
            const nodes: FlowchartNode[] = [
              {
                id: conditionId,
                label: this.escapeString(conditionSyntaxNode.text),
                shape: "diamond",
            ...
            this.locationMap.push({
              start: conditionSyntaxNode.startIndex,
              end: conditionSyntaxNode.endIndex,
              nodeId: conditionId,
            });

            const consequenceId = this.generateNodeId("ternary_true");
            const consequenceFlowchartNode = this.createFlowchartNode(
              consequenceId,
              `${targetText} = ${this.escapeString(
                consequenceSyntaxNode.text
              )}`,
              "rect",
              this.nodeStyles.process,
              consequenceSyntaxNode
            );
            nodes.push(consequenceFlowchartNode);
            this.locationMap.push({
            ...
            const alternativeFlowchartNode = this.createFlowchartNode(
              alternativeId,
              `${targetText} = ${this.escapeString(
                alternativeSyntaxNode.text
              )}`,
              "rect",
              this.nodeStyles.process,
              alternativeSyntaxNode
            );
            nodes.push(alternativeFlowchartNode);
            this.locationMap.push({
              start: statement.startIndex,
              end: statement.endIndex,
  ...
  private processWithStatement(
    ...
    return this.createProcessResult(nodes, edges, withEntryId, [
      { id: withEntryId },
    ]);
  }
