 ...
 */
export async function analyzePythonModule(code: string, filePath: string): Promise<ModuleInfo> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('=') && !line.includes('==') && !line.includes('!=') && 
        ...
        line.indexOf('=') > 0 && !line.trim().startsWith('#')) {
    ...
    for (const match of funcCallMatches) {
      ...
      if (!isBuiltinFunction(callName) && !line.startsWith('def ') && !line.startsWith('class ')) {
...
function isBuiltinFunction(name: string): boolean {
  const builtins = [
    ...
    'abs', 'round', 'sorted', 'reversed', 'open', 'input', 'type',
